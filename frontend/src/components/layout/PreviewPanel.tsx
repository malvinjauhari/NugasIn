...
function getLaprakSections(logoPath: string | null): PreviewSection[] {
  ...
  if (logoPath) {
    sections.push({
      title: "Cover",
      elements: [
        { type: "image", src: `${API_BASE}/assets/${logoPath.split("/").pop()}`, alt: "Logo Universitas" },
      ],
    });
  }
...
function getMakalahSections(logoPath: string | null): PreviewSection[] {
  const sections: PreviewSection[] = [];
...
function getSectionsForTemplate(templateName: string | null, logoPath: string | null): PreviewSection[] {
  switch (templateName) {
    case "laprak":
      return getLaprakSections(logoPath);
    case "makalah":
      return getMakalahSections(logoPath);
    case "logbook":
...
export function PreviewPanel() {
  const { metadata, templateName, pageSettings, logoPath } = useDocumentStore();

  const sections = getSectionsForTemplate(templateName, logoPath);
